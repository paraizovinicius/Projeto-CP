...
public class UI {
    ...
    public void draw(Graphics2D g2){
        ...
        if(gp.gameState == gp.NORMAL_STATE){
            Image map_icon = LoadSave.getIcon(1);
            Image openMap_icon = LoadSave.getIcon(2);
            ...
            if(gp.mouseMotion.coordXmouseMotion >=35 && gp.mouseMotion.coordXmouseMotion <= 35+577/10 && gp.mouseMotion.coordYmouseMotion >= 708 && gp.mouseMotion.coordYmouseMotion <= 708+700/10){
                g2.drawImage(openMap_icon, 30, 703, null);
            } else{
                g2.drawImage(map_icon, 35, 708, null); 
            }
        }

        // MAP STATE -- 1
        if(gp.gameState == gp.MAP_STATE){
            Image image = LoadSave.getIcon(3);
            int width = image.getWidth(null);
            int height = image.getHeight(null);
            g2.drawImage(image, (gp.SCREEN_WIDTH - width)/2, (gp.SCREEN_HEIGHT - height)/2, null);
        }
    }
